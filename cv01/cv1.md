...
- **CIELE** predmetu:
    1. komunikácia medzi procesmi (IPC): (A) _vzdialená_ a (B) _lokálna_.
...
- deklarácia štruktúry `struct` - každý prvok musí byť prevedený medzi `Host-byte-order` a `Network-byte-order` samostatne (úloha programátora) => šírka dátovej zbernice CPU je ale pevná (32b / 64b) => musím vždy načítať celú šírku z pamäte => údaje menšie než táto šírka sú následne vybrané.
